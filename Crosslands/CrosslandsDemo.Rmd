...
renderPlotly({
  if(input$Var == "Depth"){
    ...
    tidy$datetime <- floor_date(tidy$datetime, "minute")
    ...
    range <- data.frame(datetime=seq.POSIXt(mindate, maxdate, by=60))
    wdepth <- left_join(range, wdepth, by="datetime")
    wdepth$ribmin <- 0
    wdepth <- wdepth %>% mutate(ribmax = ifelse(is.na(Depth),0,Depth))

    p <- ggplot(wdepth,aes(x=datetime, y=Depth)) + 
            geom_line(colour="steelblue") +
            geom_ribbon(aes(x=datetime, ymin=ribmin, ymax=ribmax), fill="steelblue", alpha = 0.5) +
            theme_minimal() + 
