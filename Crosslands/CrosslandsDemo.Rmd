...
mindate <- min(cache_wdepth$datetime)

renderPlotly({
    p3 <- ggplot(cache_wdepth,aes(x=datetime, y=Depth)) + 
    ...
    p3
    plotly_p3 <- ggplotly(p3, dynamicTicks=TRUE) %>% layout(margin=list(r=20, l=60, t=20, b=50))
    plotly_p3
})
...
renderPlotly({
  if(input$Var == "Depth"){
    ...
    p <- ggplot(test,aes(x=datetime, y=Depth)) + 
            geom_point(colour="#5499C7", size=0.5) +
            geom_line(colour="grey70", alpha=0.5, linetype="dotted") +
            theme_minimal() + 
            theme(panel.background = element_rect(colour=NA, fill="grey97")) + 
